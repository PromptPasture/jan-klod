...
| Slice 1b — MVP parity | `in-progress` — core skeleton + all three host caps (`host-log`/`host-config`/`host-http`) done; both guests built (`store-memory` + `provider-openai`); the component test harness drives both guests offline; the **exit gate is met** — the host-side agent loop drives one turn (config → completion → store) over the Component Model, live (`make turn`) and offline (`make harness`); only the supply-chain CI gates remain before Slice 1b closes |
...
- [x] **Exit gate:** config-driven load → lifecycle → OpenAI-compatible completion through a sandboxed component + in-memory store, all over the Component Model. Wired into one driven flow by the host-side agent loop ([src/core/host/src/agent.rs](../../../src/core/host/src/agent.rs)) — the seed of the future `manager.agent-loop` guest: `run_turn` resolves the enabled provider + store from `jan-klod.yaml`, instantiates each as its category world, drives `init`→`start`, completes the prompt through `llm-provider`, then persists the prompt+response into `memory-store` (reading it back to confirm). `host-http` is **injected** as an `HttpFn`, so the `jan-klod --turn "…"` binary runs the turn **live** (`make turn`, real blocking client) while the offline exit-gate test ([src/core/host/tests/exit_gate.rs](../../../src/core/host/tests/exit_gate.rs)) drives the *same* `run_turn` against a canned reply — config → completion → store with no network or api key, under `make harness`. This replaces the probe's hand-rolled caller; `provider_probe` remains the live single-leg path (`make probe`). The richer routing/fallback/tool loop stays deferred to the `manager.agent-loop` guest (fed by the top-level agent config the core preserves verbatim) — this driver is deliberately the minimal one-provider/one-store/one-turn seed.
