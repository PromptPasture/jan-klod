...
| Slice 1b — MVP parity | `in-progress` — core skeleton + all three host caps (`host-log`/`host-config`/`host-http`) done; both guests built (`store-memory` + `provider-openai`); a live OpenAI-compatible completion runs end-to-end via `provider_probe`; the component test harness drives both guests offline (`make harness`); supply-chain CI gates remain |
