...
- [x] Host caps: `host-log`, `host-config`, `host-http` ✓. HTTP is blocking (`ureq` 3, sync, no `tokio`; async contingency unneeded). Capability-neutral impl in [`jan_klod_core::http`](../../../src/core/src/http.rs); 4xx/5xx/transport → typed errors.
