...
| 11 — UX polish | `not-started` | [PLAN.md](../2026-07-03-shipping-plan/PLAN.md). REST surface migrated to resource model (`POST /turn` → `POST /session/:id/message`, add `GET /sessions`, `POST /sessions`, `GET /session/:id`); per-token streaming in TUI (Phase 6 carry-forward); session list + resume; workspace auto-detection (default `$PWD`). Gate: resume a prior session with streaming in the TUI. |
