...
| 16 — Capability manifest + signed registry | `in-progress` | [#38](https://github.com/PromptPasture/jan-klod/issues/38). Vision decision 4. **16a-1 done 2026-09-09** ([#86](https://github.com/PromptPasture/jan-klod/issues/86)): every guest ships a manifest generated from its own imports, verified against the component and guarded by a self-test; nothing reads one until 16a-2 (#87). Gate: a component whose manifest omits a capability it imports is refused at boot; a tampered download is refused by `ext install`; an install from a static index fixture works offline; WIT `api-version` mismatch is a clear error. |
...
- **16a — Manifest + boot cross-check.** Split in two ([#50](https://github.com/PromptPasture/jan-klod/issues/50)),
  ...
  - **16a-2 — the boot cross-check** ([#87](https://github.com/PromptPasture/jan-klod/issues/87)):
    introspect a component's real imports, refuse an imported-but-undeclared
    capability with a message naming the interface, compare `api-version`, and
    make `extensions.allow-unmanifested` the named widening. Also the point at
    which [#46](https://github.com/PromptPasture/jan-klod/issues/46)'s deferred
    per-turn sandbox warning gets the fact it needs.
