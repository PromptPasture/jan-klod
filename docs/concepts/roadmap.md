...
| 5 — Distribution & ops | `in-progress` | Planning recorded 2026-07-02: [PLAN.md](../decisions/2026-07-02-phase5-distribution-ops/PLAN.md). Slices: 5a tiny Go supervisor/updater (blue/green stage→flip→health-check→rollback; adds `GET /health` to the REST surface), 5b Configurator + curated bundles, 5c exit gate. Deploy unit = host-side core binary + provider/interceptor guests (store/REST/telegram/delegation are now in-core, not guests). |
