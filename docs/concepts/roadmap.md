...
| 26 — Trusted delivery | `not-started` | [#236](https://github.com/PromptPasture/jan-klod/issues/236). [Vision — The Enterprise Box](../decisions/2026-09-18-enterprise-box/Vision.md), decision 3. Phase 16 built signing and a registry index and shipped both inert: every distribution's `config.yaml` has `registry.trusted-keys` empty and no `registry.url` (the root `config.yaml` names one), and `git tag` returns nothing, so `ext install` refuses by construction and `install.sh` has nothing to fetch. Slices: keypair and CI secret ([#93](https://github.com/PromptPasture/jan-klod/issues/93), 26a), the SBOM published and covering `src/web` ([#238](https://github.com/PromptPasture/jan-klod/issues/238)), `v0.1.0` tagged with Pages serving the registry ([#239](https://github.com/PromptPasture/jan-klod/issues/239)), the distribution configs trusting the published key ([#240](https://github.com/PromptPasture/jan-klod/issues/240)), offline install proven ([#241](https://github.com/PromptPasture/jan-klod/issues/241)). Blocked on a human — the key has to be generated before any of it runs. Gate: an operator on a network reaching nothing but a self-hosted model verifies the archive against the published key, installs an extension by name from the bundled index, runs a turn; a tampered archive is refused with the reason named. |
| 27 — Secrets, and one place redaction happens | `not-started` | [#237](https://github.com/PromptPasture/jan-klod/issues/237). [Vision](../decisions/2026-09-18-enterprise-box/Vision.md), decision 5. Keys are `${VAR}` expansion into a config string (`src/config/src/lib.rs`) and nothing keeps one out of the event log, an SSE frame or a rebuilt transcript. New `wit/host-secrets.wit` (`get(name) -> result<string, secrets-error>`, the error one of `not-found`, `denied`, `backend`; manifest-gated like every other capability — [#242](https://github.com/PromptPasture/jan-klod/issues/242)), backends for macOS Keychain ([#243](https://github.com/PromptPasture/jan-klod/issues/243)) and Linux libsecret with the environment as fallback only when D-Bus is absent ([#244](https://github.com/PromptPasture/jan-klod/issues/244)), and redaction at `PersistingSink` — the one point every persisted and streamed string passes — with patterns read from the `interceptor-guardrails` rule set that is already data, failing closed on its own terms because the dispatcher fails open outside `tool-call` ([#245](https://github.com/PromptPasture/jan-klod/issues/245)). Independent of 26 and of 21. Gate: a provider reads its key through `host-secrets`, the model echoes it verbatim, and neither log nor live client nor restarted transcript contains it, while an extension that did not declare the capability cannot read it at all; a malformed redaction set refuses to start. |
| 28 — Identity at the boundary | `not-started` | [Vision](../decisions/2026-09-18-enterprise-box/Vision.md), decision 4. `authorised()` in `src/host/src/serve.rs` compares one shared secret; it establishes no principal, and every caller sees every session. The credential resolves to a principal at the guard through a `serve.principals` table (today's single token is `operator`), `jk-session` scopes sessions to it, the principal reaches `user-turn` in `wit/interceptor.wit` as an optional field, `interceptor-rbac` is a reference guest with roles as data, off by default, composing with the per-session grants as two gates in series, and Telegram — which admits every chat id today — gets `telegram:<id>` as its principal behind an empty-by-default allowlist (28e). **Phase 21 is not a prerequisite** — transport already lives in `jan-klod-host`; 21e ([#183](https://github.com/PromptPasture/jan-klod/issues/183)) changes the package name, which is a merge-order concern. Identity asserted by a client or by a guest is declined. Gate: two principals each run a turn, each sees only their own sessions, and the RBAC guest stops one at `before-loop` rather than at the tool, naming the principal in the record; an unlisted Telegram sender is refused before a session exists. |
...
record; the tracker rows above hold the state. Umbrella and slice issues are filed for Phase 26
([#236](https://github.com/PromptPasture/jan-klod/issues/236)) and Phase 27
([#237](https://github.com/PromptPasture/jan-klod/issues/237)); 28–31 are unfiled. The record was
audited against the code on 2026-09-19 before the issues were cut, and its corrections are in the
record and in the rows above (see the changelog).
...
needing no new contract, and across sessions — run-scoped, then per principal — because a memory
that ends with the session adds nothing the context window does not hold — Phase 30a. The **curation half does not**: semantic search, consolidation
