...
- **16c — `ext install` with provenance.** Split three ways
  ...
  - **16c-2 — install from a URL** ([#92](https://github.com/PromptPasture/jan-klod/issues/92)),
    through the egress policy. Only adds where the bytes come from; 16c-1
    settled the policy it inherits — a digest is required wherever the
    signature is waived, so nothing lands with no evidence.
  - **16c-3 — sign first-party releases and publish the key**
    ([#93](https://github.com/PromptPasture/jan-klod/issues/93)). Not urgent for
    a checkable reason: `git tag` is empty and there are no releases, so there
    is no unsigned release in the wild. It must land before the first one, which
    is a different thing from soon — until it does, `registry.trusted-keys` has
    nothing to put in it and every install needs `--allow-unsigned`.
