...
created: 2026-06-29
updated: 2026-09-11
status: v0.1.0 complete (Phases 1–12 done, nothing tagged or released yet); Harness as a Platform under way — Phases 13, 14 and 15 done (13c deferred to Phase 17, 15d to a Windows environment), 16 in progress, 17–18 not-started
---
...
| 18 — Ecosystem ports | `in-progress` | [#40](https://github.com/PromptPasture/jan-klod/issues/40). Vision decision 6. Needs 13 (done). **18a done 2026-09-11** (#56): `jan-klod-gateway mcp` serves `ask`, `session_list` and `session_get` over MCP stdio — a method-name adapter over the envelope `rpc` already speaks, so no SDK and no new dependency. Half the gate is met: an MCP client lists and calls a core-exposed tool offline; the ACP half is 18b. Note a turn over MCP cannot write or run commands — there is nobody to answer a confirmation, so each takes its default, which is a refusal. Remaining: 18b (ACP server side), 18c (`registry-mcp` long-lived children). Gate: an ACP client fixture runs a turn against the core; an MCP client lists and calls a core-exposed tool — both offline. |
...
- **15c — Linux Landlock backend. Done 2026-09-10** ([#48](https://github.com/PromptPasture/jan-klod/issues/48)).
  ...
  Unlike 15b, **CI runs these tests** — `ubuntu-latest` has Landlock — so this is
  the half of Phase 15 a regression cannot slip past.
