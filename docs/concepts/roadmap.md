...
created: 2026-06-29
updated: 2026-09-13
status: v0.1.0 complete (Phases 1–12 done, nothing tagged or released yet); Harness as a Platform under way — Phases 13, 14, 15, 16, 17 and 18 done (13c **superseded by Phase 20**, 15d deferred to a Windows environment, 18c standing alone), 19 done (19a–19h), 20 open
---
...
| 25 — Self-extension | `open` | [#176](https://github.com/PromptPasture/jan-klod/issues/176). **Posture half done** ([#192](https://github.com/PromptPasture/jan-klod/issues/192)): the `self-extend` distribution enables `tool-shell` and compiles a wasm guest with `writable: ["."]`, `network: false` and `require: true` — the jail unchanged. The "toolchain outside the jail" this row used to name was never true: both backends allow reads and `process-exec` wholesale. What actually blocked a build was `/dev/null` and `TMPDIR`, fixed for every command in [#212](https://github.com/PromptPasture/jan-klod/issues/212). Remaining: install from a session without a guest that can write `ext/` ([#213](https://github.com/PromptPasture/jan-klod/issues/213)), load without a restart ([#214](https://github.com/PromptPasture/jan-klod/issues/214) — the lifecycle verbs do not exist yet), and the cheaper rung nobody specified, installing a signed extension the model found ([#218](https://github.com/PromptPasture/jan-klod/issues/218)). Defaults and signing rules unchanged. Gate: "write me a tool that does X" ends with it callable in the same session, no restart. |
