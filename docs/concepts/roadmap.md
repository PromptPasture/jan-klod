...
| 4 — Clients & integrations | `in-progress` | Planning recorded 2026-07-02: [PLAN.md](../decisions/2026-07-02-phase4-clients-integrations/PLAN.md). Slices: 4a UI client (thin REST client first, then `ratatui` TUI), 4b `host-socket` + `chat-telegram`, 4c `agent-*` ACP delegation, 4d exit gate. Clients are separate processes over the Phase 3 REST surface. |
