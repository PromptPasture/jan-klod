...
| A parked turn | Ends within heartbeat of client disappearing, or at timeout; default is denial. **Over ACP no timeout**: answer on same pipe, closed pipe detected at once, refuses | `JK_ANSWER_TIMEOUT_SECS` (REST only) | `core::serve::PromptDriver::wait_for_answer`; `core::acp::PipeAsker::ask` (stdio) | `host/tests/it/prompt_disconnect.rs::a_disconnected_client_does_not_hold_the_turn_open`, `core/src/acp.rs::a_closed_pipe_while_parked_refuses_rather_than_hanging` |
| Who may answer a confirmation (`acp`) | **Editor only** through `session/request_permission` — options are gate's own answers. Unreadable answers refuse: `cancelled`, `selected` with no `optionId`, empty, JSON-RPC error, closed pipe. MCP port: nobody can answer, all take default denial | none — port is the grant, subprocess editor spawned | `core::acp::PipeAsker::ask` builds options and maps outcome; `core::acp::AcpDriver` hands answer to permission gate | `host/tests/it/acp.rs::an_editor_that_grants_permission_gets_the_write`, `::an_editor_that_refuses_permission_prevents_the_write`, `core/src/acp.rs::a_permission_request_carries_the_gates_own_options`, `::an_answer_that_cannot_be_read_refuses` |
