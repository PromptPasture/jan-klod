...
| The terminal client's ask dialog (Slice 19f) | `default` is a **denial**, so a rendering mistake here has a security consequence. The initial selection is `default`'s own index in `options`, found by search rather than assumed first; if `default` is not among `options` at all, nothing is selected and an explicit choice is required. The answer sent is byte-identical to the chosen entry — never normalised — and carries the **notification's own session**, not the client's current one. `Esc` closes the dialog without answering; the prompt stays queued and can be reopened, and `Enter` while the dialog is closed reopens it rather than answering blind. A second `ask` while one is open queues rather than stacking, and answering the first leaves the second's own selection untouched. Every mark survives `Mode::Mono`: the default is named in the row's own text, not only by colour | none — this is the client's rendering of `interceptor-permission`'s own answers, which decides nothing itself | `jan-klod-ui`'s `app::App::ask`/`take_answer`/`prompt_move`/`prompt_jump` hold the queue and the selection; `blocks::prompt_dialog` renders it; `tui.rs`'s `dialog_keys` and `enter_means` route the keys | `ui/src/app.rs::the_initial_selection_is_defaults_index_wherever_it_sits`, `::the_answer_is_byte_identical_to_the_chosen_option`, `::the_answer_carries_the_prompts_own_session`, `::a_second_ask_queues_behind_the_first_and_neither_disturbs_the_other`, `::esc_closes_without_answering_and_the_prompt_survives`, `ui/src/blocks.rs::the_dialog_says_what_silence_means_and_names_the_default`, `::monochrome_still_marks_the_default_option_in_text`, `ui/src/tui.rs::esc_closes_the_dialog_without_sending_turn_answer`, `::the_answer_is_sent_with_the_prompts_own_session_not_the_clients` |
