...
| A **model** driving a long-lived child (`tool-proc`) | Denied. The tool is an extension like any other — off unless `extensions.tool.proc.enabled`, and inert without `execution.long-lived` entries, so two grants are needed rather than one. **The model names a child and never supplies a command**: that stays in config, which is what keeps this narrower than `execution.enabled`. `host-process.granted` discloses the granted *names* so a model can ask for one and a refusal can say what exists; what a name runs is never returned. A child the model forgets is reaped when the instance goes, and its unread output is bounded by `execution.output-cap` | `extensions.tool.proc.enabled: true`, **and** `execution.long-lived` entries | `src/extensions/tool-proc` keeps `name -> handle` in its own instance; admission is still `core::host_process::ProcessRunner::spawn_long_lived`, unchanged by this tool | `host/tests/it/tool_proc.rs::a_child_starts_in_one_turn_is_read_in_the_next_and_stops_in_a_third`, `::an_ungranted_name_is_refused_and_the_refusal_names_what_is_granted`, `::a_child_the_model_forgot_does_not_outlive_the_runtime`, `core/src/host_process.rs::an_unread_childs_stdout_is_bounded_by_the_output_cap` |
...
| The WebSocket surface (`GET /ws`) | The same rule as the REST routes, in the same place: the upgrade happens behind the one layer every route passes through, so the socket needed no second implementation and cannot drift from one. Refused before the upgrade, not after — an unauthenticated client never gets a connection to send frames on | `JAN_KLOD_TOKEN` | `host::serve::guard`, ahead of `host::serve::socket`'s upgrade | `host/tests/it/ws.rs::the_socket_is_refused_without_the_token`, whose control is `::a_client_connects_and_negotiates_the_protocol` — without a connection that works, "refused" and "no such route" are the same observation |
