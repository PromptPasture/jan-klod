...
fn find_component(engine: &Engine) -> Option<Component> {
    ...
    if !path.exists() {
        eprintln!("skipping: tool-find.wasm not staged — run `make ext`");
        return None;
    }
...
fn glob_walks_the_workspace_and_respects_the_jail() {
    ...
    let Some(component) = find_component(&engine) else { return };
    ...
    std::fs::write(workspace_dir.join("src").join("util").join("helper.rs"), "// h").unwrap();
    std::fs::write(workspace_dir.join("target").join("debug").join("build.rs"), "// b").unwrap();
    ...
    assert_eq!(paths, vec!["src/main.rs", "src/util/helper.rs"], "recursive + pruned: {found}");

    // Naming the pruned directory opts back into it.
    let named = tool.invoke(r#"{"pattern":"target/**/*.rs"}"#).expect("glob succeeds");
    assert_eq!(named.lines().collect::<Vec<_>>(), vec!["target/debug/build.rs"]);

    // `path` scopes the walk; the pattern is relative to it, results are not.
    let scoped = tool.invoke(r#"{"pattern":"*.rs","path":"src/util"}"#).expect("glob succeeds");
    assert_eq!(scoped.lines().collect::<Vec<_>>(), vec!["src/util/helper.rs"]);

    // No match is an explicit statement, not an empty result.
    let empty = tool.invoke(r#"{"pattern":"**/*.zig"}"#).expect("glob succeeds");
    assert!(empty.contains("no files match"), "empty result must say so: {empty}");

    // The walk cannot leave the jail, even when asked directly.
    let escape = tool.invoke(r#"{"pattern":"*","path":".."}"#);
    assert!(escape.is_err(), "a root outside the workspace must be denied: {escape:?}");
...
fn tool_find_is_default_deny_without_a_workspace() {
    ...
    let mut tool =
        ToolExtension::instantiate(&engine, "tool.find", &component, None, ProcessRunner::disabled())
            .expect("tool instantiates");
