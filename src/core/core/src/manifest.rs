//! What a component declares it needs, and whether the component agrees.
//!
...
//! component that needs more than it admits to is either mislabelled or lying,
//! and both are worth stopping before it runs.
//!
//! The other direction is not a refusal. Declaring a capability that
//! `config.yaml` does not grant is fine and grants nothing — every capability
//! is default-deny at the call, so an over-declaring manifest asks for more
//! permission than it receives and simply does not get it. Refusing that would
//! make a manifest a second place to keep grants in step, which is the opposite
//! of the point.
//!
//! Parsed with `toml` rather than by hand. That was measured rather than
//! assumed: `wasmtime-internal-cache` already depends on it, so the crate is
//! compiled in every build already and adding it here costs **zero** new
//! packages. Hand-parsing would have been defensible for the generated shape
//! and a liability the first time a third-party author writes one by hand —
//! comments after values, single quotes, an inline array — and this is a
//! security check, so mis-parsing is worse than a dependency that is already
//! present.
//!
//! Fields are read off a `toml::Table` rather than deserialized into a struct,
