...
//! ## What it does today
//!
//! `tool-call` only: arguments matching a rule `block` with the rule's reason,
//! or `ask`. Redaction at `after-response`/`finalize` and refusal at
//! `select-context` are the next slice (#196 configures it, #177 tracks the rest).
//!
...
mod component {
    use crate::rules::{Act, RuleError, Rules};
    ...
    impl Interceptor for Component {
        ...
        fn intercept(input: InterceptInput) -> Result<Decision, InterceptorError> {
            ...
            let verdict = RULES.with(|r| match &*r.borrow() {
                // The host fails closed at `tool-call` on `Err`, which is the
                // behaviour a misconfigured guardrail should have.
                Err(_) => Err(InterceptorError::Internal),
                Ok(rules) => Ok(rules.review_tool_call(&call.name, &call.arguments)),
            })?;
            ...
            match (verdict.act, input.answer) {
                (Act::Block, _) => {
                    log(
                        LogLevel::Info,
